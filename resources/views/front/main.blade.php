                    ...
                    @forelse($products as $item)
                        <div class="col-4">
                            <div class="card w-100">
                                ...
                                <div class="card-body">
                                    <h5 class="card-title">{{ mb_strimwidth($item->name, 0, 20, "...") }}</h5>
                                    <p class="card-text">{{ mb_strimwidth($item->description, 0, 50, "...") }}</p>
                                    <p>$ {{ $item->price }}</p>
                                    <a href="{{ route('view', $item->id) }}" class="btn btn-primary">Посмотреть</a>
                                    <a href="{{ route('addToCart', $item->id) }}" class="btn btn-primary">Купить</a>
                                </div>
                            </div>
                        </div>
                    @empty
