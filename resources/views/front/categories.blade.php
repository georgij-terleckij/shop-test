                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        @foreach($data->breadcrumb as $item)
                            <li class="breadcrumb-item"><a href="{{ route('catalog',$item['1']) }}">{{ $item[0] }}</a></li>
                        @endforeach
                        <li class="breadcrumb-item active" aria-current="page">Каталог {{ $data->name }}</li>
                    </ol>
                </nav>
                <h1>Каталог{{ $data->name }}</h1>
                <ul>
                    @foreach($data->catalogs as $catalog)
                        <li><a href="{{ route('catalog', $catalog['id']) }}">{{ $catalog['name'] }}</a></li>
                    @endforeach
                </ul>
